<template>
	...
	<div class="top-nav">
        ...
        <div class="btnList" v-show="urlParams.subShow&&urlParams.isSend">
            <ul class="clear">
                <li class="fl">
                    <button @click="ignoreResume(job)" v-show="showBtn(job)">忽略</button>
                </li>
                <li class="fl" v-show="showBtn(job)">
                    <button @click="handResume(job)">接受</button>
                </li>
                <li class="fl" v-show="urlParams.isSend==1">
                    <button>已接受</button>
                </li>
            </ul>
        </div>
...
<script>
	...
	export default{
        ...
        created(){
            ...
            // alert(JSON.stringify(this.urlParams))
            let data={
		...
		methods:{
      showBtn(job){
              if(this.urlParams.isSend!=0){
                return false
              }
              let time=new Date(job.startDate+'00:00:00').getTime();
              let now=new Date().getTime();
              if(time>now&&this.urlParams.isSend==0){
                return true
              }
              if(time<=now){
                return false
            ...
            handResume(job){
              ...
              }else  if(sessionStorage.getItem('resume')+''=='true'){
                    MessageBox({
                          title: '提示',
                          message:'现在去完善简历？',
